...
function initContactForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) form.reset(), alert('Message envoyé avec succès !');
...
function initContentProtection() {
    ...
    document.addEventListener('keydown', e => { if (['F12','i','j','c','u'].includes(e.key.toLowerCase()) && (e.ctrlKey || e.key === 'F12')) showWarning(); });
    ...
    setInterval(() => {
        ...
        if ((widthDiff || heightDiff) && !devToolsOpen) devToolsOpen = true, showWarning();
    }, 1000);
}
